...
public class AddIfMin extends ServerCommand implements Executable {
    public AddIfMin() {
        super();
    ...
    public Response calling() {
        ...
        if (spm.compareTo(Objects.requireNonNull(getCollectionRepository().getCollectionStream().min(Comparator.naturalOrder()).orElse(null))) <= 0) {
            new Add().calling(spm);
        } else {
